...
```{r Student_Enrollment, fig.cap="Increases in Student Enrollment", fig.height=5, fig.width=7}
EnrollTab %>%
...
```{r Higher_Ed_Expenses_2014, fig.cap="Distribution of Higher Education Expenses in 2014", fig.height=5, fig.width=7}
expensedata %>% 
...
```{r IPEDS_Tenure_Pct_over_Time, fig.cap="Percentage Tenured Faculty in Postsecondary Education (IPEDS)"}
Tenure_Sector_table %>%
  filter(UNIT=='FACULTY') %>% 
  gather(TENURE, PCT, 2:15) %>% 
  filter(TENURE %in% c("FORPROF","PUB4YRTOTAL","PUB2YR","NONPROF4YRTOTAL","NONPROF2YR")) %>%
...
```{r IPEDS_Facclass_Pct_over_Time, fig.cap="Percentage of Faculty Types in Postsecondary Education (IPEDS)"}
...
Contract_table %>% 
  ...
  labs(title = "Instructional Faculty Contracts in 2013", y = "Number of Instructors", caption = "Evans & Furstenberg. NCES IPEDS Finance Component (Delta Project) 2014.") +
  theme(panel.background = element_rect(fill = 'aliceblue', colour = 'black'))
```
